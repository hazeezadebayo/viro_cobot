...
class PublisherJointTrajectory(Node):
    def __init__(self):
        super().__init__("joint_traj_ctrlr_test")
        ...
        self.get_logger().info('Publishing {} goals on topic "{}" every {} s'.format( len(goal_names), publish_topic, wait_sec_between_publish  ) )
    ...
    def timer_callback(self):
        if self.starting_point_ok:
            ...
            # if the duration is small, the created interpolated waypoint is small.
            # point.time_from_start = Duration(sec=50)
            # point.time_from_start = Duration(sec=2)
            time_from_start = 0.2
            seconds = int(time_from_start)
            nanoseconds = int((time_from_start - seconds) * 1e9)
            # print(f"Seconds: {seconds}, Nanoseconds: {nanoseconds}")
            point.time_from_start = Duration(sec=int(seconds), nanosec=nanoseconds) # point.time_from_start = Duration(sec=chronometer)
