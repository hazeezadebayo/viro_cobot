  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("EthercatDriver"), "initializing ......");
  master_.setCtrlFrequency(control_frequency_);

  for (auto i = 0ul; i < ec_modules_.size(); i++) {
    // RCLCPP_INFO(rclcpp::get_logger("EthercatDriver"), ("Starting ...3alias... " + ec_module_parameters_[i]["alias"]).c_str());
    // RCLCPP_INFO(rclcpp::get_logger("EthercatDriver"), ("Starting ...3pos... " + ec_module_parameters_[i]["position"]).c_str());
    // RCLCPP_INFO(rclcpp::get_logger("EthercatDriver"), ("Starting ...3get... " + std::to_string(reinterpret_cast<std::uintptr_t>(ec_modules_[i].get()))).c_str());
    master_.addSlave(
